...
def _beam_search_with_backtracking(args, target_client, prm_client, draft_tokenizer, target_tokenizer, prm_tokenizer, draft_model, prompt, problem):
    ...
    while beam and num_step < args.max_steps:
        ...
        beam, newly_completed = _check_completion_beam(beam, args, draft_tokenizer, target_tokenizer, stats)
...
def _unified_search_step_with_backtracking(
    ...
    # 3) No good candidates → TARGET INTERVENTION on the ORIGINAL BEAM (not on "bad children")
...
def _evaluate_with_prm_log_scores_backtracking(candidates, problems, prm_client, prm_tokenizer, args, stats, all_rewards):
    ...
    return candidates
def _check_completion_beam(beam, args, draft_tokenizer, target_tokenizer, stats):
...
def main(target_client, prm_client, draft_client, draft_tokenizer, target_tokenizer, prm_tokenizer, draft_model, data_name, args):
    ...
    for epoch in range(max_func_call):
        ...
        for prompt, problem in zip(prompts, problems):
            ...
            # Properly aggregate model stats for all problems
            if model_stats is None:
                # Deep copy to avoid reference issues
                import copy
                model_stats = copy.deepcopy(mstats)
                # Ensure prefetch instrumentation keys exist even if mstats came from a codepath
                # that doesn't populate them (e.g., non-tree/no-prefetch flows).
                prefetch_defaults = {
                    "prefetch_consume_time": 0.0,
                    "prefetch_consume_calls": 0,
                    "prefetch_collect_wait_time": 0.0,
                ...
                for k, v in prefetch_defaults.items():
                    model_stats.setdefault(k, v)
            else:
                # Aggregate timing stats
                model_stats["draft_time"] += mstats["draft_time"]
                model_stats["target_time"] += mstats["target_time"]
                model_stats["prm_time"] += mstats["prm_time"]
                # Aggregate token counts
                model_stats["draft_tokens"] += mstats["draft_tokens"]
                model_stats["target_tokens"] += mstats["target_tokens"]
                model_stats["prm_tokens"] += mstats["prm_tokens"]
                # Aggregate input/output tokens
                model_stats["draft_input_tokens"] += mstats["draft_input_tokens"]
                model_stats["draft_output_tokens"] += mstats["draft_output_tokens"]
                model_stats["target_input_tokens"] += mstats["target_input_tokens"]
                model_stats["target_output_tokens"] += mstats["target_output_tokens"]
                model_stats["prm_input_tokens"] += mstats["prm_input_tokens"]
                model_stats["prm_output_tokens"] += mstats["prm_output_tokens"]
                # Extend batch lists
                model_stats["draft_batches"].extend(mstats["draft_batches"])
                model_stats["target_batches"].extend(mstats["target_batches"])
                model_stats["prm_batches"].extend(mstats["prm_batches"])
                # Aggregate prefetch instrumentation
                prefetch_keys = [
