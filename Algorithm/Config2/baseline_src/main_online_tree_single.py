...
def parse_args():
    ...
    parser.add_argument("--target_model_name_or_path", default="Qwen/Qwen2.5-1.5B-Instruct", type=str)
...
def _consume_target_prefetch(nodes, target_client, args, stats):
    if not args.enable_target_prefetch:
        return [], list(nodes)
...
def _unified_search_step_with_backtracking(
    ...
    if target_children:
        target_token_count = len(target_tokenizer.encode(target_children[0].single_response))
    
    stats["step_attempts"].append({
        "step": len(stats["step_attempts"]),
        "draft_tokens": draft_token_count,   # 这里就是你想看的：被 Reject 的 Draft Token 数
        "target_tokens": target_token_count, # 这里是替代它的 Target Token 数
...
def _target_intervene_from_nodes(nodes, target_client, target_tokenizer, args, stats, override_expansion_factor=None):
    ...
    prefetched_children, pending_nodes = _consume_target_prefetch(nodes, target_client, args, stats)
...
def main(target_client, prm_client, draft_client, draft_tokenizer, target_tokenizer, prm_tokenizer, draft_model, data_name, args):
    ...
    sample_count = len(input_prompts)
    sample_token_counts = [(0, 0, 0) for _ in range(sample_count)]
    sample_turn_info = [[] for _ in range(sample_count)]
    sample_rewards = [[] for _ in range(sample_count)]
    sample_start_times = [None for _ in range(sample_count)]
    sample_end_times = [None for _ in range(sample_count)]
    sample_model_stats = [_init_stats() for _ in range(sample_count)]
    ...
    for epoch in range(max_func_call):
        ...
        # [MultiEpochBookkeepingFix] current_prompts can be a subset after epoch 0.
        problems = [samples[item[0] // args.n_sampling]["question"] for item in current_prompts]
        ...
        for (sample_idx, _), prompt, problem in zip(current_prompts, prompts, problems):
            problem_counter += 1
            if sample_start_times[sample_idx] is None:
                sample_start_times[sample_idx] = time.time()
            t0 = time.time()
            if args.beam_search:
            ...
            outputs.extend(out)
            sample_end_times[sample_idx] = t1
            ...
            if tcount:
                tc = tcount[0]
                prev_tc = sample_token_counts[sample_idx]
                sample_token_counts[sample_idx] = tuple(prev_tc[j] + tc[j] for j in range(3))
            if tinfo:
                sample_turn_info[sample_idx].extend(tinfo[0])
            if areward:
                sample_rewards[sample_idx].extend(areward[0])
            ...
            for key in scalar_stat_keys:
                ...
                sample_model_stats[sample_idx][key] += mstats.get(key, 0)
            for key in list_stat_keys:
                model_stats[key].extend(mstats.get(key, []))
                sample_model_stats[sample_idx][key].extend(mstats.get(key, []))
            # Per-problem latency
    ...
    for i, sample in enumerate(samples):
        ...
        latency = (
            sample_end_times[i] - sample_start_times[i]
            if i < len(sample_end_times) and sample_start_times[i] is not None and sample_end_times[i] is not None
            else None
        )
        # Compute steps and target intervention from turn_info (step_info)
        step_info = sample_turn_info[i] if i < len(sample_turn_info) else []
        ...
        # [MultiEpochBookkeepingFix] Use per-sample stats accumulated across epochs.
        current_mstats = sample_model_stats[i] if i < len(sample_model_stats) else {}



        sample.update(
            {"code": code, "pred": preds, "report": reports, 
             "token_counts": sample_token_counts[i], "turn_info": step_info, "reward": sample_rewards[i],
             "latency": latency, 
