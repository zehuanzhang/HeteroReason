...
FPGA_PER_TOKEN_MS = 5.74/2     # linear term, per generated draft token
FPGA_ATTN_COEF_MS = 0.0024   # quadratic (attention) term coefficient
MAX_DRAFT_LATENCY_MS = 2500


# communication model
COMM_FIXED_LATENCY_MS = 0.060
COMM_BANDWIDTH_GBPS = 5.0
TOKEN_INDEX_BYTES = 4
...
FPGA_Power_Active = 120 # scaling to 300Mhz, 45W measured at 200MHz
FPGA_Power_Load_Idle = 120 
...
def communication_time_ms(draft_output_tokens):
    """One packed draft-token-index transfer to PRM."""
    n_tokens = max(float(draft_output_tokens), 0.0)

    # No draft output means there is no token-index transfer.
    if n_tokens <= 0.0:
        return 0.0

    payload_bytes = n_tokens * TOKEN_INDEX_BYTES
    payload_time_ms = (
        payload_bytes / (COMM_BANDWIDTH_GBPS * 1e9) * 1000.0
    )
    return COMM_FIXED_LATENCY_MS + payload_time_ms
...
def compute_our_or_strong(steps, draft_ms, target_overlap, args, is_fpga=True):
    ...
    for i, s in enumerate(steps):
        ...
        is_last = i == last
        current_communication_ms = communication_time_ms(
            s.draft_out_tokens
        )
        ...
        if i == 0:
            ...
            if prev_rescue:
                ...
                prev_communication_ms = communication_time_ms(
                    prev.draft_out_tokens
                )
                prev_prm_path_ms = prev_communication_ms + prev.prm_ms
                front = max(draft_ms[i], prev_prm_path_ms)

        
        if not rescue:
            current_prm_path_ms = current_communication_ms + s.prm_ms
            cost = front + (current_prm_path_ms if is_last else 0.0)
            ...
            prm_head_ms = current_communication_ms + half_prm
            cost = front + prm_head_ms + s.target_ms
            if is_last:
                cost += half_prm
            ...
            prm_head_ms = current_communication_ms + p
            cost = max(front + prm_head_ms, t)
            ######### Calculate Power ######### 
            draft_power += (
                draft_load_idle * t
            ...
            half_prm = s.prm_ms / 2.0
            prm_head_ms = current_communication_ms + half_prm
            cost = max(front + prm_head_ms, s.target_ms)
            ######### Calculate Power ######### 
...
def compute_weak(steps):
    ...
    return [s.draft_ms + communication_time_ms(s.draft_out_tokens) + s.prm_ms + s.target_ms for s in steps]
