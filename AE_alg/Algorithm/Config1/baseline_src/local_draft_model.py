...
class LocalDraftModel:
    ...
    def completions_create(self, 
        ...
        # Always use generate_batch with n completions per prompt
        # batch_outputs = self.generate_batch(prompts, temperature, top_p, max_tokens, stop, max_batch_size, n=n)

        # Now, batch_outputs is a list of lists: [[completion1, ...], ...] for each prompt
        # We need to call vLLM directly to get finish_reason/stop_reason, so let's do that for accurate info
        # Re-run vLLM for the batch to get the full output objects (as in local_target_model)
        stop_tokens = stop if isinstance(stop, list) else [stop] if stop else None
        sampling_params = SamplingParams(
            temperature=temperature,
            top_p=top_p,
