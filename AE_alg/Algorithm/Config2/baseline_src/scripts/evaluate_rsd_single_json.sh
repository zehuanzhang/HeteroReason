...
#TARGET_MODEL="/mnt/ccnas2/bdp/zr523/RSD/Qwen2.5-Math-1.5B-Instruct" #"/mnt/ccnas2/bdp/zr523/RSD/Qwen2.5-1.5B-Instruct"
...
# DRAFT_MODEL="/mnt/ccnas2/bdp/zr523/RSD/Qwen2.5-1.5B-Instruct"
# [Target1p5BChange] New experiment family uses the 1.5B target.
TARGET_MODEL="/mnt/ccnas2/bdp/zr523/RSD/Qwen2.5-1.5B-Instruct"
...
if [ -z "$PRM_TAG" ]; then
    PRM_TAG="$(basename "$PRM_MODEL")"
fi
PRM_TAG_SAFE="$(printf '%s' "$PRM_TAG" | sed -e 's#[/ .]#_#g' -e 's#[^A-Za-z0-9_.-]##g')"
PRM_THRESHOLD_TAG="${PRM_THRESHOLD//./p}"
EXPERIMENT_SUFFIX="${PRM_TAG_SAFE}_thr${PRM_THRESHOLD_TAG}"
...
echo "PRM threshold: $PRM_THRESHOLD"
echo "PRM tag: $PRM_TAG_SAFE"
...
if [ "$MODE" = "local" ]; then
    BASE_CMD="python main_online_tree_single.py \
        ...
        --use_local_models \
        --output_dir \"outputs/${APPROACH}_${EXPERIMENT_SUFFIX}_single_local\" \
    ...
    BASE_CMD="python main_online_tree_single.py \
        ...
        --output_dir \"outputs/${APPROACH}_${EXPERIMENT_SUFFIX}_single_server\" \
        --prompt_type \"qwen25-math-cot\" \
        --apply_chat_template \
        --prm_threshold $PRM_THRESHOLD \
        --max_steps 100 \
...
if [ "$NSYS" = "1" ]; then
    mkdir -p outputs
    NSYS_CMD="nsys profile --trace=cuda,nvtx,osrt --trace-fork-before-exec=true --kill=sigterm --output=outputs/nsys_report_${APPROACH}_${EXPERIMENT_SUFFIX}_single_${MODE}_$(date +%Y%m%d_%H%M%S)"
else
    NSYS_CMD=""
fi
...
case $APPROACH in
  ...
  bbeam1_prefetch_cache)
    ...
    ;;
  # [DraftPipeCancelChange] Cancellable draft pipeline: start next-step draft during PRM, abort it on PRM reject.
  bbeam1_prefetch_cache_draftpipeline_cancel)
    echo "Running with backtracking beam search + target prefetch cache + cancellable draft pipeline prefetch"
    eval "$NSYS_CMD $BASE_CMD --beam_search --beam_width 1 --beam_expansion_factor 1 --search_temperature 0.0 --enable_backtracking --enable_target_prefetch --enable_prefetch_cache --enable_draft_pipeline_prefetch"
    ;;
  # Compatibility alias for the earlier draftpipeline approach name in this copied experiment directory.
  bbeam1_prefetch_cache_draftpipeline)
    echo "Running with backtracking beam search + target prefetch cache + cancellable draft pipeline prefetch (compat alias)"
    eval "$NSYS_CMD $BASE_CMD --beam_search --beam_width 1 --beam_expansion_factor 1 --search_temperature 0.0 --enable_backtracking --enable_target_prefetch --enable_prefetch_cache --enable_draft_pipeline_prefetch"
    ;;
  bbeam2)
    echo "Running with beam search"
