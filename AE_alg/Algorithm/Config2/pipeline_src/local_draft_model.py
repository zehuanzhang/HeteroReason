...
class DraftGenerationJob:
    """Internal record for one in-flight draft generation request."""
    ...
    def update_from_request_output(self, request_output) -> None:
        with self._lock:
            ...
            if request_output.finished:
                # Some vLLM builds mark the request finished before all completion
                # objects expose finish_reason. Treat the latest cumulative output
                # as the finished payload in that case.
                for idx, payload in self.partial_outputs.items():
                    self.finished_outputs.setdefault(idx, payload)
...
class LocalDraftCompletions:
    """Wrapper matching the OpenAI-style completions.create(...) signature."""

    def __init__(self, draft_model: "LocalDraftModel") -> None:
        self._draft_model = draft_model

    def create(self, **kwargs: Any):
        return self._draft_model.completions_create(**kwargs)


class LocalDraftModel:
    """
    Local draft model wrapper with cancellable async generation.

    [DraftPipeCancelChange] The draft-pipeline experiment needs to abort a
    next-step draft as soon as PRM rejects the current step. Both synchronous
    completions and async prefetches share one worker queue so vLLM is not
    driven concurrently from multiple Python threads.
    """
    ...
    def __init__(
        ...
        self.completions = LocalDraftCompletions(self)
        self._request_counter = itertools.count()
        self._command_queue: "queue.Queue[Dict[str, Any]]" = queue.Queue()
        self._jobs_lock = threading.Lock()
        self._jobs: Dict[str, DraftGenerationJob] = {}
        self._completed_jobs: Dict[str, DraftGenerationJob] = {}
        self._stop_event = threading.Event()
        self._worker_thread = threading.Thread(
            target=self._worker_loop,
            name="LocalDraftModelWorker",
            daemon=True,
        )
    ...
    def completions_create(
        ...
        **_: Any,
        ...
        for batch_start in range(0, len(prompts), max_batch_size):
            batch_prompts = prompts[batch_start : batch_start + max_batch_size]
            job_ids: List[str] = []
            ...
            for local_prompt_idx, job_id in enumerate(job_ids):
                job = self._wait_for_completion(job_id)
                if job.error:
                    raise RuntimeError("Draft generation failed") from job.error
                ...
                prompt_idx = batch_start + local_prompt_idx
    ...
    def generate(
        ...
        return [choice.text for choice in sorted(response.choices, key=lambda item: item.index)]
    ...
    def generate_batch(
        ...
        grouped: List[List[str]] = [[] for _ in processed_prompts]
        for choice in sorted(response.choices, key=lambda item: item.index):
            prompt_idx = int(choice.index) // max(n, 1)
            if 0 <= prompt_idx < len(grouped):
                grouped[prompt_idx].append(choice.text)
        return grouped

    # ------------------------------------------------------------------
    # Public API (draft-pipeline helpers)
    # ------------------------------------------------------------------
    ...
    def _schedule_job(self, prompt: str, sampling_params: SamplingParams) -> str:
        job_id = f"draft-{next(self._request_counter)}"
        prompt_tokens = len(self.tokenizer.encode(prompt)) if self.tokenizer else 0
        job = DraftGenerationJob(job_id, prompt, sampling_params, prompt_tokens)
    ...
    def _wait_for_completion(self, job_id: str) -> DraftGenerationJob:
        job = self._get_job(job_id)
        if job is None:
    ...
    def _get_job(self, job_id: str) -> Optional[DraftGenerationJob]:
        with self._jobs_lock:
    ...
    def _handle_command(self, command: Dict[str, Any]) -> None:
        ...
        if kind == "add":
            job: DraftGenerationJob = command["job"]
            try:
    ...
    def _locate_job(self, request_id: str) -> Optional[DraftGenerationJob]:
        with self._jobs_lock:
